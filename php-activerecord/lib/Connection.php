...
abstract class Connection
{
	...
	public $last_query;
        /**
         * The last query populated
         * @var string
         */
        public $last_query_prep;
	/**
	 * Switch for logging.
	 *
	 * @var bool
	...
	public function query($sql, &$values=array())
	{
		...
		$this->last_query = $sql;
                ...
                // PDO wont handle arrays in prepare statements, so we check for arrays and prepare them into strings
                // this breaks the 
                foreach ($values as &$value){
                    // convert arrays to strings due to array not being supported
                    if (is_array($value)){
                        foreach ($value as &$val){
                            if (is_string($val)) 
                                $val = addslashes ($val); // add slashes as is necessary
                        }
                        
                        $value = implode(',', $value); // implode with ,
                    }
                }
                $this->last_query_prep = $this->interpolateQuery($sql, $values);
                //if ($sql == 'SELECT * FROM `collection` WHERE `account_id`=? AND `parent_collection_id` IS ?')
                //    die($this->last_query_prep);
                
                // SELECT * FROM `collection` WHERE `account_id`=1 AND `parent_collection_id` IS NULL
                // SELECT * FROM `collection` WHERE `account_id`=1 AND `parent_collection_id` IS NULL
